...
def data_collection_orderbook(title, upbitcu, currency, upbit_currency, duration):
    orderbook_ask = []
    orderbook_bid = []
    orderbook_ask_upbit = []
    orderbook_bid_upbit = []
    ...
    while i < duration:
        ...
        np.save(f'odb_{title}_{upbitcu}_orderbook1.npy', final)
        np.save(f'odb_{title}_{upbitcu}_upbit_orderbook1.npy', final_upbit)
        np.save(f'odb_{title}_{upbitcu}_time_binance.npy', time_binance)
        np.save(f'odb_{title}_{upbitcu}_time_upbit.npy', time_upbit)